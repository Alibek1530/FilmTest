...
import uz.ali.filmtest.models.Backdrop
import uz.ali.filmtest.ui.detal.DetalsFilmFragment

class AdapteVideosPager(val detalsFilmFragment: DetalsFilmFragment, var model: List<Backdrop>) :
    PagerAdapter() {
    ...
    override fun instantiateItem(container: ViewGroup, position: Int): Any {
        vieww = LayoutInflater.from(container.context).inflate(R.layout.item_video_image, null)
    ...
    fun setImageOpen(position: Int) {
        val imageLink = Contents.BASE_IMAGE + model[position].file_path
        ...
        vieww.setOnClickListener {
            detalsFilmFragment.setPosIdImages(position)
        }
    }
}
